...
class Context(object):
    def __init__(self, dimensions, linear_space=False):
        self.dimensions = dimensions
        self.linear = linear_space

    def resize(self, dimensions):
        vgResizeSurfaceAM(dimensions[0], dimensions[1])
        check_error()
...
def context_new(cls, dimensions, linear_space=False):
    if cls.singleton is None:
        success = vgInitContextAM(dimensions[0], dimensions[1], linear_space)
        if not success:
